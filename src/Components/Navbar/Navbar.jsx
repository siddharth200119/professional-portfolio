...
function Navbar(props){
    ...
    return(
        <div className="flex flex-col">
            <div className="flex flex-row justify-between md:px-20 px-3 h-12 md:h-16 items-center flex-wrap backdrop-blur-xl fixed w-screen z-10">
            ...
            <div className={`fixed backdrop-blur-xl md:px-20 px-3 mt-12 flex md:hidden flex-col w-full ${open? 'max-h-96' : 'max-h-0'} overflow-hidden transition-max-height duration-500 ease-in-out z-10`}>
                {props.navbarOptions.map(option => <NavbarOption key={option.href} text={option.text} href={option.href}/>)}
            </div>
        </div>
    )
